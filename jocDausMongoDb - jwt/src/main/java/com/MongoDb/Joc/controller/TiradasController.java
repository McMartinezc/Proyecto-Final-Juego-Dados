...
public class TiradasController {
	...
	public ResponseEntity<Tirada> jugadorTiraDaus (@PathVariable("id") String id) {
	...
	public ResponseEntity<String> deleteTiradasJugador(@PathVariable("id") String id) {
	...
	public ResponseEntity<List<Tirada>> getTiradasByJugador(@PathVariable("id") String id) {

		try {
			if (service.getTiradesByUser(id) != null) {
				return new ResponseEntity<>(service.getTiradesByUser(id), HttpStatus.OK);
			} else {
				return new ResponseEntity<>(HttpStatus.NOT_FOUND);
			}
